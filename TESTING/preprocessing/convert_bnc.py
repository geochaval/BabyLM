...
for part in glob.glob(f"{SOURCE_FOLDER}/*/*.xml"):
    ...
    directory = os.path.join(TARGET_FOLDER, *part.split(os.sep)[-3:-1])
    pathlib.Path(directory).mkdir(parents=True, exist_ok=True)
    filename = os.path.join(directory, os.path.splitext(part.split(os.sep)[-1])[0])

    try:
        with open(f"{filename}_temp.md", mode="w", encoding="utf-8") as f_out:
