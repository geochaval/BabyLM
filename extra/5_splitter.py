...
def format_for_llm(articles):
    """Format articles for LLM training by adding special tokens and joining on newlines"""
    formatted_articles = []
    for article in articles:
        # Remove any existing newlines and excessive spaces
        article = ' '.join(article.split())
        # Add special tokens
        formatted = f"<s>{article}</s>"
        formatted_articles.append(formatted)
    
    # Join all articles with newlines
